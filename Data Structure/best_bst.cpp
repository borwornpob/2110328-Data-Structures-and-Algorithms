...
void insertBalanced(CP::map_bst<int, int> &bst, const std::vector<int>& nums, int start, int end) {
    if (start > end) {
        return;
    }
    int mid = start + (end - start) / 2;
    bst[nums[mid]] = nums[mid]; // Using the operator[] to insert
    insertBalanced(bst, nums, start, mid - 1);
    insertBalanced(bst, nums, mid + 1, end);
}
